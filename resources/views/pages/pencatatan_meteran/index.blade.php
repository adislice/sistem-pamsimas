...
@section('content')
  ...
  <div class="bg-white rounded-lg shadow-sm p-5">
    ...
    <form action="" method="get" class="mx-auto flex gap-2 flex-wrap md:flex-nowrap">
        <div class="flex w-72">
            <label for="bulan" class="bg-gray-50 border border-gray-300 border-r-0 px-3 rounded-s flex items-center font-medium">Bulan</label>
            <x-input-select id="bulan" placeholder="123" class="rounded-e rounded-s-none border-s-0" name="bulan">
                <option value="1">Januari</option>
            </x-input-select>
        </div>
        <div class="flex w-72">
            <label for="tahun" class="bg-gray-50 border border-gray-300 border-r-0 px-3 rounded-s flex items-center font-medium">Tahun</label>
        ...
        <div class="flex w-72">
            <label for="rt" class="bg-gray-50 border border-gray-300 border-r-0 px-3 rounded-s flex items-center font-medium">RT</label>
        ...
        <div class="flex w-72">
            <label for="rw" class="bg-gray-50 border border-gray-300 border-r-0 px-3 rounded-s flex items-center font-medium">RW</label>
        ...
        <x-button type="submit" class="rounded min-w-24">
            Oke
        </x-button>
    </form>
  </div>
          ...
          @foreach ($list_pelanggan as $row)
            <tr>
              ...
              <td class="whitespace-nowrap">
                ...
                @else
                  <span class="text-sm">Belum dicatat</span>
                @endif
              </td>
              <td class="whitespace-nowrap">
                <div class="flex gap-1">
                @if ($row->created_at == null)
                <a href="{{ route('pencatatan_meteran.catat', $row->id) . '?' . request()->getQueryString() }}" class="inline-flex items-center bg-green-500 rounded px-2 py-1 text-sm text-white">Catat</a>
                  
                @endif
                <a href="{{ route('pencatatan_meteran.show', $row->id ?? 0) }}"
                ...
                <a href="{{ route('pelanggan.edit', $row->id ?? 0) }}"
