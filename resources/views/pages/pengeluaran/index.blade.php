...
@section('content')
  <div x-data="{ modalDelete: false, deleteId: null }">
    <div class="mb-4 flex items-center">
      <h4 class="font-bold text-2xl text-gray-700">Pengeluaran</h4>
    ...
    <div class="bg-white rounded-lg shadow-sm p-3 mt-2">
      @if (isset($pengeluaran))
        <div class="flex gap-2 mb-2">
          <x-button href="{{ route('pengeluaran.create') }}" class="bg-blue-600 rounded">
          ...
          <table class="table">
            ...
            <tbody>
              @foreach ($pengeluaran as $row)
                <tr>
                  <td class="whitespace-nowrap">{{ $pengeluaran->firstItem() + $loop->index }}</td>
                  ...
                  <td class="whitespace-nowrap">
                    <div class="flex gap-1">
                      <x-button href="{{ route('pengeluaran.show', $row->id) }}"
                        class="bg-green-500 hover:bg-hover p-0 size-8 rounded"
                        title="Lihat data">
                        <x-feathericon-eye class="size-4" />
                      </x-button>
                      <x-button href="{{ route('pengeluaran.edit', $row->id) }}"
