@extends('template.dashboard_layout', ['back' => route('pengeluaran.index'), 'title' => 'Detail Pengeluaran'])

@section('content')
  <div x-data="{ modalDelete: false, deleteId: null }">
    <div class="flex">
      <div class="">
        <h4 class="mb-0 font-bold text-2xl text-gray-700">Detail Pengeluaran</h4>
        <p class="mb-4 text-gray-500">Detail pengeluaran #{{ $pengeluaran->id }}</p>
      </div>
      <div class="flex gap-1 items-center ms-auto">
        <x-button href="{{ route('pengeluaran.edit', $pengeluaran->id) }}"
          class="rounded bg-yellow-500 hover:bg-hover py-1.5 px-3">
          <x-feathericon-edit class="size-4 me-2" />
          Edit
        </x-button>
        <x-button @click="modalDelete = true; deleteId='{{ $pengeluaran->id }}'"
    ...
    <div class="bg-white rounded-lg shadow-sm p-5">
      <div class="flex flex-col">
        <div class="mb-3">
          ...
          <div class="text-gray-600">{{ $pengeluaran->nama }}</div>
        </div>
        <div class="mb-3">
          <div class="font-medium">Jumlah</div>
          <div class="text-gray-600">{{ Akaunting\Money\Money::IDR($pengeluaran->jumlah)->formatWithoutZeroes() }}</div>
        </div>
        <div class="mb-3">
            <div class="font-medium">Penanggung Jawab / Person In Charge</div>
            <div class="text-gray-600">{{ $pengeluaran->pic }}</div>
          </div>
        <div class="mb-3">
          <div class="font-medium">Tanggal</div>
          <div class="text-gray-600">{{ $pengeluaran->created_at->translatedFormat('d F Y') }}</div>
        </div>
        <div class="mb-3">
          <div class="font-medium">Keterangan</div>
          <div class="text-gray-600">{{ $pengeluaran->keterangan }}</div>
        </div>
      </div>
    </div>
